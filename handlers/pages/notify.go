...
func NotifyHandler(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet {
			forPage.Notifications, _ = getUserNotifications(db, forPage.User.ID)
			err := markNotificationsAsRead(db, forPage.User.ID)
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
			unread, err := GetUnreadNotificationCount(db, forPage.User.ID)
			log.Println(unread)
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
			forPage.UnreadNotifications = unread
